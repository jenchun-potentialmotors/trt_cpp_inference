...
class CudaMemory {
public:
    static std::unique_ptr<void, std::function<void(void*)>> allocateDevice(size_t size) {
        void* deviceMemory = nullptr;
        if (cudaMalloc(&deviceMemory, size) != cudaSuccess) {
            throw std::runtime_error("cudaMalloc failed");
        }
        return std::unique_ptr<void, std::function<void(void*)>>(deviceMemory, [](void* ptr) {
            if (ptr) cudaFree(ptr);
        });
    }

    static std::unique_ptr<void, std::function<void(void*)>> allocateHost(size_t size) {
        void* hostMemory = nullptr;
        if (cudaMallocHost(&hostMemory, size) != cudaSuccess) {
            throw std::runtime_error("cudaMallocHost failed");
        }
        return std::unique_ptr<void, std::function<void(void*)>>(hostMemory, [](void* ptr) {
            if (ptr) cudaFreeHost(ptr);
        });
    }
};

TensorrtEngine::TensorrtEngine(const Inference::Options& options) : enginePath_(options.model_path),
    runtime_(createInferRuntime(logger), [](IRuntime* runtime) { if (runtime) delete runtime; }),
    engine_(nullptr, [](ICudaEngine* engine) { if (engine) delete engine; }),
    context_(nullptr, [](IExecutionContext* context) { if (context) delete context; }),
    stream_(nullptr) {
    if (!initialize()) {
...
bool TensorrtEngine::initialize() {
    ...
    if (engineExists) {
        ...
        std::cerr << "Engine file not found, building engine is not implemented." << std::endl;
        return false;
    }
...
bool TensorrtEngine::executeInference(const std::vector<float>& inputData, std::vector<float>& outputData) const {
    if (!engine_ || !context_) {
        throw std::runtime_error("Engine or context not initialized.");
    }
    ...
    for (size_t i = 0; i < inputBuffers_.size() / 2; ++i) {
        cudaMemcpyAsync(inputBuffers_[2 * i + 1].get(), inputBuffers_[2 * i].get(), inputBufferSizes_[i], cudaMemcpyHostToDevice, stream_);
    }
    ...
    for (size_t i = 0; i < outputBuffers_.size() / 2; ++i) {
        cudaMemcpyAsync(outputBuffers_[2 * i].get(), outputBuffers_[2 * i + 1].get(), outputBufferSizes_[i], cudaMemcpyDeviceToHost, stream_);
    }
    cudaStreamSynchronize(stream_);

    outputData.resize(outputBufferSizes_[0] / sizeof(float));
    std::memcpy(outputData.data(), outputBuffers_[0].get(), outputBufferSizes_[0]);
...
bool TensorrtEngine::loadEngine() {
    ...
    engine_.reset(runtime_.get()->deserializeCudaEngine(engineData.data(), fsize));
...
size_t TensorrtEngine::getDataTypeSize(DataType dtype) {
    ...
    default: return 4; // Default to 4 (float)
    }
}
